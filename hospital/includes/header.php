...
$pageTitle = $pageTitle ?? 'Hospital Portal';
$activePage = $activePage ?? '';

// Fetch quick metrics for badges in navigation
$hospitalId = $currentHospital['id'];
$todayAppointmentsCount = 0;
$approvedAppointmentsCount = 0;
$pendingAppointmentsCount = 0;

if ($pdo && !empty($hospitalId)) {
    try {
        // Pending booking requests requiring hospital approval & scheduling
        $stmtPend = $pdo->prepare("SELECT COUNT(*) FROM bookings WHERE hospital_id = ? AND status = 'Pending'");
        $stmtPend->execute([$hospitalId]);
        $pendingAppointmentsCount = (int)$stmtPend->fetchColumn();

        // Today's appointments count
        $stmtToday = $pdo->prepare("SELECT COUNT(*) FROM bookings WHERE hospital_id = ? AND appointment_date = CURRENT_DATE AND status = 'Approved'");
        $stmtToday->execute([$hospitalId]);
        $todayAppointmentsCount = (int)$stmtToday->fetchColumn();

        // Total approved appointments ready for vaccination
        $stmtAppr = $pdo->prepare("SELECT COUNT(*) FROM bookings WHERE hospital_id = ? AND status = 'Approved' AND appointment_date >= CURRENT_DATE");
        $stmtAppr->execute([$hospitalId]);
        $approvedAppointmentsCount = (int)$stmtAppr->fetchColumn();
...
<head>
    ...
    <title><?= htmlspecialchars($pageTitle) ?> | Hospital Portal &bull; <?= APP_NAME ?></title>
